// s292_opt.c -- s292 优化版(opt)单函数测试入口, 面向 node 程序。
//
// 编译 (仅编译该内核, 在 vec-lab 目录下执行):
//   clang -O3 -march=native -DTYPE=double -I. entry/s292_opt.c common.c kernels/s292.c -o s292_opt.exe
//   kernels/s292.c 实现 s292_opt() 后追加 -DHAVE_S292_OPT 启用真实优化版;
...
int s292_orig(void);
int s292_opt(void);                  /* 实现于 kernels/s292.c */

#ifndef HAVE_S292_OPT
/* 占位实现: 优化函数尚未提供时保证可链接 (约定返回 1 = 未实现) */
int s292_opt(void) { return 1; }
#endif
...
int main(void)
{
  ...
  /* 3) 容差比对 + JSON 报告 */
  return lab_report_opt("s292", rc_orig, rc_opt, t_opt, cs_orig, t_orig, (TYPE)0.0);
}
